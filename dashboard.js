...
class TimetableGenerator {
    constructor(rooms, professors, studentGroups, courses) {
        ...
        this.evolutionMetrics = {
            fitnessHistory: [],           // Fitness por generación
            bestFitnessByGeneration: [],  // Mejor fitness por generación
            avgFitnessByGeneration: [],   // Fitness promedio por generación
            diversityByGeneration: [],    // Diversidad genética por generación
            operatorStats: {              // Estadísticas de operadores genéticos
                crossoverSuccess: [],     // Tasa de éxito de cruce por generación
                mutationSuccess: [],      // Tasa de éxito de mutación por generación
                repairSuccess: []         // Tasa de éxito de reparaciones por generación
            },
            convergenceMetrics: {         // Métricas de convergencia
                stagnationPeriods: [],    // Periodos sin mejora
                improvementRates: []      // Tasas de mejora entre generaciones
            },
            constraintsSatisfaction: {},  // Satisfacción de restricciones por generación
            populationSnapshots: []       // Capturas de población cada n generaciones
        };

        // Configuración de captura de métricas
        this.metricsCaptureInterval = 10;   // Capturar cada 10 generaciones
        this.populationSnapshotInterval = 100; // Guardar población cada 100 generaciones
    }

    captureMetrics(population, generation) {
        ...
        this.evolutionMetrics.fitnessHistory.push({
            generation,
            bestFitness,
            avgFitness
        });

        this.evolutionMetrics.bestFitnessByGeneration.push(bestFitness);
        this.evolutionMetrics.avgFitnessByGeneration.push(avgFitness);

        // Capturar métricas cada cierto intervalo para reducir el tamaño de datos
        if (generation % this.metricsCaptureInterval === 0) {
            // Calcular diversidad (usando varianza de fitness como aproximación simple)
            const fitnessVariance = this.calculatePopulationVariance(population);
            this.evolutionMetrics.diversityByGeneration.push({
                generation,
                diversity: fitnessVariance
            });

            // Capturar satisfacción de restricciones
            this.captureConstraintsSatisfaction(population, generation);
        }

        // Guardar población completa cada cierto número de generaciones
        if (generation % this.populationSnapshotInterval === 0) {
            // Limitar el tamaño almacenando solo el top 10%
            const snapshotSize = Math.max(3, Math.floor(population.length * 0.1));
            this.evolutionMetrics.populationSnapshots.push({
                generation,
                chromosomes: population.slice(0, snapshotSize).map(c => ({
                    fitness: c.fitness,
                    assignmentsCount: c.classAssignments.length,
                    scoreDistribution: this.calculateScoreDistribution(c)
                }))
            });
        }

        // Calcular tasa de mejora si no es la primera generación
        if (generation > 0) {
            const prevBestFitness = this.evolutionMetrics.bestFitnessByGeneration[generation - 1];
            const improvementRate = (bestFitness - prevBestFitness) / prevBestFitness;

            this.evolutionMetrics.convergenceMetrics.improvementRates.push({
                generation,
                rate: improvementRate
            });

            // Detectar estancamientos (tasa de mejora muy pequeña)
            if (improvementRate < 0.001 && improvementRate >= 0) {
                // Verificar si ya estamos en un periodo de estancamiento
                const lastStagnation = this.evolutionMetrics.convergenceMetrics.stagnationPeriods.length > 0
                    ? this.evolutionMetrics.convergenceMetrics.stagnationPeriods[this.evolutionMetrics.convergenceMetrics.stagnationPeriods.length - 1]
                    : null;

                if (lastStagnation && lastStagnation.endGeneration === generation - 1) {
                    // Extender el periodo existente
                    lastStagnation.endGeneration = generation;
                    lastStagnation.duration++;
                } else {
                    // Crear un nuevo periodo de estancamiento
                    this.evolutionMetrics.convergenceMetrics.stagnationPeriods.push({
    ...
    // Calcular varianza del fitness en la población
    calculatePopulationVariance(population) {
        const fitnesses = population.map(c => c.fitness);
        const meanFitness = fitnesses.reduce((sum, f) => sum + f, 0) / fitnesses.length;
        const squaredDiffs = fitnesses.map(f => Math.pow(f - meanFitness, 2));
        return squaredDiffs.reduce((sum, d) => sum + d, 0) / fitnesses.length;
    }

    // Calcular distribución de puntuaciones en un cromosoma
    calculateScoreDistribution(chromosome) {
        const distribution = [0, 0, 0, 0, 0, 0]; // para puntuaciones 0-5

        chromosome.classAssignments.forEach(assignment => {
            const score = Math.round(assignment.score);
            if (score >= 0 && score <= 5) {
                distribution[score]++;
            }
        });
    ...
    // Capturar satisfacción de restricciones
    captureConstraintsSatisfaction(population, generation) {
        // Analizar el mejor cromosoma para satisfacción de restricciones
        const bestChromosome = population[0];

        // Categorías de restricciones a analizar
        const constraintCategories = {
            roomCapacity: { fulfilled: 0, total: 0 },
            consecutiveSlots: { fulfilled: 0, total: 0 },
            specificRoom: { fulfilled: 0, total: 0 },
            timeRestriction: { fulfilled: 0, total: 0 },
            professorAvailability: { fulfilled: 0, total: 0 },
            groupConflicts: { fulfilled: 0, total: 0 }
        };

        // Analizar cada asignación
        for (const assignment of bestChromosome.classAssignments) {
            // Capacidad del aula
            constraintCategories.roomCapacity.total++;
            const totalStudents = assignment.course.studentGroups.reduce((sum, group) => sum + group.size, 0);
            if (assignment.room.capacity >= totalStudents) {
                constraintCategories.roomCapacity.fulfilled++;
            }

            // Slots consecutivos
            if (assignment.course.duration > 1 && assignment.course.requiresConsecutiveSlots) {
                constraintCategories.consecutiveSlots.total++;
                const slots = assignment.getAllTimeSlots();
                const isConsecutive = slots.length === assignment.course.duration &&
                    slots.every((slot, index) =>
                        index === 0 ||
                ...
                if (isConsecutive) {
                    constraintCategories.consecutiveSlots.fulfilled++;
                }
            }

            // Aula específica
            if (assignment.course.requiredRoomId !== null) {
                constraintCategories.specificRoom.total++;
                if (assignment.room.id === assignment.course.requiredRoomId) {
                    constraintCategories.specificRoom.fulfilled++;
                }
            }

            // Restricción horaria para grupos específicos
            const restrictedGroups = ["G7", "G8", "G9", "G10", "G11", "G12", "G13", "G14"];
            if (assignment.course.studentGroups.some(g => restrictedGroups.includes(g.id))) {
                constraintCategories.timeRestriction.total++;
                const slots = assignment.getAllTimeSlots();
                const allWithinRestriction = slots.every(slot => slot.hourIndex < 4);
                if (allWithinRestriction) {
                    constraintCategories.timeRestriction.fulfilled++;
                }
            }

            // Disponibilidad del profesor (simplificado)
            constraintCategories.professorAvailability.total++;
            if (bestChromosome.isProfessorAvailableForAssignment(assignment)) {
                constraintCategories.professorAvailability.fulfilled++;
            }

            // Conflictos de grupo (simplificado)
            constraintCategories.groupConflicts.total++;
            if (bestChromosome.areStudentGroupsAvailableForAssignment(assignment)) {
                constraintCategories.groupConflicts.fulfilled++;
            }
        }

        // Guardar métricas de restricciones
        this.evolutionMetrics.constraintsSatisfaction[generation] = constraintCategories;
    }

    // Registrar estadísticas de operadores genéticos
    registerOperatorStats(operatorType, success, generation) {
        if (!this.evolutionMetrics.operatorStats[operatorType]) {
            this.evolutionMetrics.operatorStats[operatorType] = [];
        }

        // Buscar estadística existente para esta generación
        let genStat = this.evolutionMetrics.operatorStats[operatorType].find(s => s.generation === generation);

        if (!genStat) {
            genStat = { generation, attempts: 0, successes: 0, rate: 0 };
            this.evolutionMetrics.operatorStats[operatorType].push(genStat);
        }
        ...
        genStat.rate = genStat.successes / genStat.attempts;
    }
    ...
    // Método para guardar métricas evolutivas en un archivo JSON
    saveEvolutionMetrics() {
        const jsonData = JSON.stringify(this.evolutionMetrics, null, 2);

        // Si estamos en un entorno Node.js, guardar en archivo
        if (typeof require !== 'undefined' && require('fs')) {
            try {
                require('fs').writeFileSync('evolucion-data.json', jsonData, 'utf8');
                console.log("Archivo 'evolucion-data.json' con métricas evolutivas guardado en el directorio actual.");
            } catch (error) {
                console.error("Error al guardar el archivo 'evolucion-data.json':", error);
            }
        } else {
            console.log("Guardado de archivo no disponible en este entorno (requiere Node.js). Los datos de evolución están disponibles en memoria.");
        }
    ...
    generateTimetable() {
        ...
        for (let generation = 0; generation < this.maxGenerations; generation++) {
            ...
            // Capturar métricas de esta generación
            this.captureMetrics(population, generation);
        ...
        // Guardar métricas evolutivas en un archivo
        this.saveEvolutionMetrics();
    ...
    crossover(parent1, parent2) {
        ...
        // Registrar estadísticas del operador
        const generation = this.evolutionMetrics.fitnessHistory.length;
        this.registerOperatorStats('crossoverSuccess', (successfulAssignments / totalAssignments) > 0.8, generation);
    ...
    mutate(chromosome) {
        ...
        // Registrar éxito de mutación (si mejoró el fitness)
        const generation = this.evolutionMetrics.fitnessHistory.length;
        this.registerOperatorStats('mutationSuccess', chromosome.fitness > preFITNESS, generation);

        return chromosome.fitness > preFITNESS;
    }
